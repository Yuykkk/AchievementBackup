...
def trigger_external_restore(backup_folder_name):
    ...
    token = f"achievementbackup_full_restore_{int(time.time())}"
    ...
    label_sources = [
        ("userdata", "userdata", os.path.join(STEAM_PATH, "userdata")),
        ("appcache_stats", "appcache/stats", os.path.join(STEAM_PATH, "appcache", "stats")),
        ("depotcache", "depotcache", os.path.join(STEAM_PATH, "depotcache")),
        ("lua", "config/lua", os.path.join(STEAM_PATH, "config", "lua")),
        ("stplug-in", "stplug-in", os.path.join(STEAM_PATH, "config", "stplug-in")),
    ]
    restore_labels = [label for folder, label, _target in label_sources if os.path.exists(os.path.join(backup_src, folder))]
    tree_jobs = [
        {
            "source": os.path.join(backup_src, folder),
            "target": target,
            "label": label,
        }
        for folder, label, target in label_sources
        if os.path.isdir(os.path.join(backup_src, folder))
    ]
    external_jobs = []
    try:
        meta_path = _meta_path(backup_src)
        ...
        for item in meta.get("externalFiles") or []:
            ...
            if os.path.exists(source) and _is_under(backup_src, source) and _is_under(root, target):
                external_jobs.append({
                    "source": source,
                    "target": target,
                    "label": "saves externos",
                })
    except:
        external_jobs = []
    if external_jobs and "saves externos" not in restore_labels:
        restore_labels.append("saves externos")

    payload = {
        "backupSrc": backup_src,
        "steamExe": steam_exe,
        "steamPath": STEAM_PATH,
        "flagFile": flag_file,
        "failureFile": failure_file,
        "summary": ", ".join(restore_labels or ["backup da Steam"]),
        "treeJobs": tree_jobs,
        "externalJobs": external_jobs,
    }
...
def write_failure(path, reason, label, target, source=""):
    try:
        with open(path, "a", encoding="utf-8") as f:
            f.write(f"{reason}|{label}|{target}|{source}\n")
    except Exception:
        pass
...
def progress(title, current, total, extra=""):
    total = max(total, 1)
    pct = int((current * 100) / total)
    done = int((current * 28) / total)
    bar = "#" * done + "-" * (28 - done)
    os.system("cls")
    print("AchievementBackup :: RESTORE COMPLETO")
    print(title)
    print()
    print(f"[{bar}] {pct}%")
    print()
...
def main():
    ...
    failure_file = payload["failureFile"]
    flag_file = payload["flagFile"]
    try:
    ...
    tree_jobs = payload.get("treeJobs") or []
    external_jobs = payload.get("externalJobs") or []
    total = len(external_jobs)
    for job in tree_jobs:
        source = job.get("source")
        if os.path.isdir(source):
            total += sum(1 for _ in iter_files(source))
    total = max(total, 1)
    ...
    progress("Fechando processos da Steam", done, total)
    for proc in ("steam.exe", "steamwebhelper.exe", "steamerrorreporter.exe"):
        subprocess.run(["taskkill", "/F", "/IM", proc], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(3)

    for job in tree_jobs:
        source_root = job["source"]
        target_root = job["target"]
        label = job["label"]
        ...
        for source in iter_files(source_root):
            ...
            progress(f"Restaurando {label}", done, total, rel)
            try:
                safe_copy(source, target)
                ok += 1
            except Exception as exc:
                fail += 1
                write_failure(failure_file, type(exc).__name__, label, target, source)
            done += 1

    for job in external_jobs:
        source = job["source"]
        target = job["target"]
        label = job.get("label") or "saves externos"
        progress("Restaurando saves externos", done, total, os.path.basename(target))
        try:
    ...
    progress("Abrindo Steam novamente", done, total)
    steam_exe = payload.get("steamExe")
    steam_started = False
    if steam_exe and os.path.exists(steam_exe):
        try:
            subprocess.Popen([steam_exe], cwd=os.path.dirname(steam_exe))
            time.sleep(5)
            ...
            steam_started = "steam.exe" in (check.stdout or "").lower()
        except Exception as exc:
            write_failure(failure_file, type(exc).__name__, "Steam", steam_exe, "")
    if not steam_started:
        fail += 1
        write_failure(failure_file, "START STEAM", "Steam", steam_exe or "steam.exe", "")

    flag = f"backup|{payload.get('summary') or 'backup da Steam'}|{ok}|{total}|{fail}"
    os.makedirs(os.path.dirname(flag_file), exist_ok=True)
    with open(flag_file, "w", encoding="utf-8") as f:
        f.write(flag)

    progress("Restore concluido", total, total, f"Arquivos aplicados: {ok} de {total} | Falhas: {fail}")
    print("A Steam foi chamada novamente." if steam_started else "Nao consegui confirmar a Steam aberta.")
    time.sleep(8)
    return 0
...
'''
    bat_content = [
        ...
        f"color {terminal_color}",
        "title AchievementBackup - Restauracao Completa",
        f'"{sys.executable}" "{worker_path}" "{payload_path}"',
        "if errorlevel 1 (",
        "  echo.",
        "  echo O restore encontrou um erro antes de finalizar.",
        "  echo Esta janela ficou aberta para voce conseguir ver o motivo.",
        "  pause",
        ")",
        f'del /F /Q "{payload_path}" >nul 2>&1',
        f'del /F /Q "{worker_path}" >nul 2>&1',
        '(goto) 2>nul & del "%~f0"',
    ]
    try:
        ...
        _log_event("ERROR", f"Failed starting Python full restore worker: {exc}")
