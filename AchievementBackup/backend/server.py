...
def trigger_external_restore(backup_folder_name):
    ...
    bat_content = [
        ...
        "set /a STEP=0",
        "set /a STEPS=8",
        f'set "LOGFILE={_b(os.path.join(PLUGIN_ROOT, "log.txt"))}"',
        f'echo [%DATE% %TIME%] Restore de backup iniciado: {_b(backup_folder_name)} >> "%LOGFILE%"',
    ...
    bat_content.extend([
        ...
        'echo [%DATE% %TIME%] Restore de backup finalizado ^| OK=%OK% ^| total=%TOTAL% ^| falhas=%FAIL% >> "%LOGFILE%"',
        "set /a STEP=STEPS-1",
    ...
    bat_content.extend([
        ...
        "if not exist \"%SRC%\" (echo IGNORADO: origem ausente & echo [%DATE% %TIME%] IGNORADO backup origem ausente ^| parte=%LABEL% ^| origem=\"%SRC%\" >> \"%LOGFILE%\" & goto :eof)",
        "set /a COUNT=0",
        ...
        "  copy /Y \"%%F\" \"!OUT!\" >nul",
        "  if errorlevel 1 (set /a FAIL+=1 & echo [%DATE% %TIME%] FALHA backup copiar ^| parte=!LABEL! ^| origem=\"%%F\" ^| destino=\"!OUT!\" >> \"%LOGFILE%\") else (set /a OK+=1)",
        "  set /a FILE_DONE+=1",
